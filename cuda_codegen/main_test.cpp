...
int main(void){
    ...
    for (unsigned i = 0; i < 10; ++i) {
        int diff = fabs(Result_1763_0_host[i] - result[i]) / fabs(result[i]);
        if (diff > 0.0001) {
            std::cout << "output: " << Result_1763_0_host[i] << ", expect: " << result[i] << std::endl;
	    std::cout << "***********failed************* \n";
        }
    }
    */
   /*
    ...
    fwrite(Result_1763_0_host, sizeof(float) * 10, 1, fp);
    fclose(fp);
    */
    //free context
