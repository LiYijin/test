...
{
    std::ifstream bin_file("./Constant/Constant_2080_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
{
    std::ifstream bin_file("./Constant/Constant_2677_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
{
    std::ifstream bin_file("./Constant/Constant_2809_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
{
    std::ifstream bin_file("./Constant/Constant_2935_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
{
    std::ifstream bin_file("./Constant/Constant_2023_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
{
    std::ifstream bin_file("./Constant/Constant_3122_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
{
    std::ifstream bin_file("./Constant/Constant_3076_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
...
// Node name:	Constant_2790
// Description:	Constant
// Input:
// Output:
//	- name: Constant_2790_0	type: float	shape: Shape{1, 64, 16, 16}
void Constant_float_cuda_Constant_2790(cudaStream_t stream, float* output0)
{
    std::ifstream bin_file("./Constant/Constant_2790_0.bin" , std::ios::in | std::ios::binary);
    if(bin_file.fail())
    {
    	printf("Load Constant_2790_0 failed.\n");
    	exit(1);
    }
    char* tmp_mem = new char[65536];
    bin_file.read(tmp_mem, 65536);
...
// Node name:	AvgPool_888
// Description:	AvgPool
// Input:
//	- name: Relu_885_0	type: float	shape: Shape{1, 192, 32, 32}
// Output:
//	- name: AvgPool_888_0	type: float	shape: Shape{1, 192, 16, 16}
void AvgPool_float_float_cuda_lib_AvgPool_888(cudnnHandle_t cudnn_handle, float* input0, float* output0)
{
    cudnnTensorDescriptor_t input_desc;
    CUDNN_SAFE_CALL(cudnnCreateTensorDescriptor(&input_desc));
    CUDNN_SAFE_CALL(cudnnSetTensor4dDescriptor(input_desc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 1, 192, 32, 32));
    cudnnTensorDescriptor_t output_desc;
    CUDNN_SAFE_CALL(cudnnCreateTensorDescriptor(&output_desc));
    CUDNN_SAFE_CALL(cudnnSetTensor4dDescriptor(output_desc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 1, 192, 16, 16));
...
extern "C" __launch_bounds__(10) __global__ void Add_float_float_float_cuda_Add_1762(float* input0, float* input1, float* output0)
{
...
extern void Add_float_float_float_cuda_Add_1762_Call(const dim3 &grids, const dim3 &blocks, unsigned mem, cudaStream_t stream, float* input0, float* input1, float* output0) {
    Add_float_float_float_cuda_Add_1762<<<grids, blocks, mem, stream>>>(input0, input1, output0);
}
...
// Fused functions:
// Slice_float_float_cuda_Slice_1671<<<dim3(128, 1, 1), dim3(64, 1, 1), 0, 0>>>(BatchNormInference_1669_0, Slice_1671_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1678<<<dim3(64, 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1672_0, Constant_344_0, DepthwiseConv2dNative_1678_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1676<<<dim3(64, 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1672_0, Constant_216_0, DepthwiseConv2dNative_1676_0);
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1677<<<dim3(64, 1, 1), dim3(128, 1, 1), 0, 0>>>(Relu_1672_0, Constant_51_0, DepthwiseConv2dNative_1677_0);
// Deduped function map: <src_function_name : deduped_function_name>
// DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1677 : DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1676
...
// Output:
//	- name: Slice_1671_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __noinline__ void Slice_float_float_cuda_Slice_1671_block_kernel(float* input0, float* output0, int thread_id, int block_id, char *shared_buffer)
{
    if (thread_id >= 64){
        return;
    }
...
// Output:
//	- name: DepthwiseConv2dNative_1678_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __noinline__ void DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1678_block_kernel(float* input0, float* input1, float* output0, int thread_id, int block_id, char *shared_buffer)
{
    if (thread_id >= 128){
        return;
    }
...
// Output:
//	- name: DepthwiseConv2dNative_1676_0	type: float	shape: Shape{1, 128, 8, 8}
__device__ __noinline__ void DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1676_block_kernel(float* input0, float* input1, float* output0, int thread_id, int block_id, char *shared_buffer)
{
    if (thread_id >= 128){
        return;
    }
...
extern "C" __global__  void BlockFusionKernel_float_float_float_float_float_float_float_float_float_cuda_Slice_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_168(float* input0, float* input1, float* input2, float* input3, float* input4, float* output0, float* output1, float* output2, float* output3)
{

    if ((int)blockIdx.x >= 0 && (int)blockIdx.x <= 127)
    {
        Slice_float_float_cuda_Slice_1671_block_kernel(input0, output0, threadIdx.x, blockIdx.x - 0 + 0, NULL);
    }
    else if ((int)blockIdx.x >= 128 && (int)blockIdx.x <= 191)
    {
        DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1678_block_kernel(input1, input2, output1, threadIdx.x, blockIdx.x - 128 + 0, NULL);
    }
    else if ((int)blockIdx.x >= 192 && (int)blockIdx.x <= 255)
    {
        DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1676_block_kernel(input1, input3, output2, threadIdx.x, blockIdx.x - 192 + 0, NULL);
    }
    else if ((int)blockIdx.x >= 256 && (int)blockIdx.x <= 319)
    {
        DepthwiseConv2dNative_float_float_float_cuda_DepthwiseConv2dNative_1676_block_kernel(input1, input4, output3, threadIdx.x, blockIdx.x - 256 + 0, NULL);
    }
...
extern void BlockFusionKernel_float_float_float_float_float_float_float_float_float_cuda_Slice_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_168_Call(const dim3 &grids, const dim3 &blocks, unsigned mem, cudaStream_t stream, float* input0, float* input1, float* input2, float* input3, float* input4, float* output0, float* output1, float* output2, float* output3) {
    BlockFusionKernel_float_float_float_float_float_float_float_float_float_cuda_Slice_DepthwiseConv2dNative_DepthwiseConv2dNative_DepthwiseConv2dNative_168<<<grids, blocks, mem, stream>>>(input0, input1, input2, input3, input4, output0, output1, output2, output3);
}
